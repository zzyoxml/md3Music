...
class _PlaylistPageState extends State<PlaylistPage> {
  ...
  void _showCommentsSheet(BuildContext context) {
    ...
    debugPrint(
      '[CommentsSheet] commentId=$commentId, commentType=$commentType',
    );
  ...
  Future<void> _fetchSongs() async {
    ...
    try {
      ...
      if (widget.isDefaultFavorite && _songs.isNotEmpty) {
        context.read<FavoritesProvider>().addFavoriteIds(
  ...
  Widget build(BuildContext context) {
    ...
    return ListenableBuilder(
      listenable:
          PlaylistPage.songFilterListenable ??
            ...
            body: Column(
              children: [
                if (_isLoading)
                  ...
                  // 歌单的元数据/封面/描述仍可见）
                  if (_error != null)
                    _buildErrorBanner(context, colorScheme, textTheme),
                      ...
                      slivers: [
                        if (!_isMultiSelectMode)
                          ...
                          ValueListenableBuilder<bool>(
                            ...
                            builder: (context, collapsed, _) => _buildAppBar(
                              context,
                              colorScheme,
                              textTheme,
                              displayPlaylist,
      ...
      child: ClipRect(
        child: LayoutBuilder(
          builder: (context, constraints) {
            ...
            // 可用高度需同时扣除 SafeArea 顶部 inset（状态栏）与 padding 上下 64，
            // 否则折叠过程中 coverSize 会按偏大的高度计算，超出 Row 真实可用高度
            // 触发 RenderFlex overflow（曾出现最多 12px 的下方溢出）。
            final safeTop = MediaQuery.paddingOf(context).top;
            final availH = (constraints.maxHeight - 64 - safeTop)
                .clamp(0.0, constraints.maxHeight);
            final coverSize = availH.clamp(0.0, 140.0);
            // 标题透明度连续映射：可用高度充足时全显示（1.0），折叠到阈值
            // 以下时淡出（0.0），实现平滑淡出而非折叠瞬间直接消失。
                    ...
                    child: Padding(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          Expanded(
                            // 文字区可视高度随 availH 收缩：超出部分由 ClipRect
                            // 裁掉并配合淡出（opacity），封面缩小时不会残留文字
                            // 超出 Row 实际高度而触发 RenderFlex overflow（黄条）。
                            child: SizedBox(
                              height: availH,
                              child: Align(
...
class _SimilarPlaylistsViewState extends State<_SimilarPlaylistsView> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (parsed.isNotEmpty) {
        debugPrint(
          '[SimilarPL] first name=${parsed.first.name} id=${parsed.first.id} gid=${parsed.first.globalCollectionId}',
        );
      }
      if (!mounted) return;
      setState(() {
  ...
  KugouPlaylistBrief? _mapSimilarPlaylist(Map<String, dynamic> item) {
    ...
    final name =
        (item['collection_name'] ?? item['name'] ?? item['specialname'] ?? '')
            .toString()
            .replaceAll(RegExp(r'<[^>]*>'), '')
            .trim();
  ...
  Widget _coverPlaceholder(ColorScheme cs) {
    return Container(
      ...
      child: const Center(child: Icon(Icons.queue_music, color: Colors.grey)),
    );
  }
}
