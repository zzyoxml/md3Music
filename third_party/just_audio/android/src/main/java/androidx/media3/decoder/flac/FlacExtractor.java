...
public final class FlacExtractor implements Extractor {
  ...
  /** 是否已产出过至少一个音频帧（EOS-before-first-sample 判别探针用）。 */
  private boolean firstSampleEmitted;
  ...
  public int read(final ExtractorInput input, PositionHolder seekPosition) throws IOException {
    ...
    try {
      ...
      if (outputSize == 0) {
        ...
        // 这里把判别信息一次性打出来：
        //   decoderState = END_OF_STREAM 且 inputPos ≈ inputLen → 源文件真的到此结束（文件截断/不完整）；
        //   decoderState = END_OF_STREAM 但 inputPos ≪ inputLen → DataSource 提前报告 EOF（读取路径问题）；
        ...
        if (!firstSampleEmitted) {
          Log.w(
              ...
              "FlacExtractor: EOS before first sample (decoderState="
                  ...
                  + input.getLength()
                  + ", bitsPerSample="
                  + (streamMetadata != null ? streamMetadata.bitsPerSample : -1)
                  + ", sampleRate="
                  + (streamMetadata != null ? streamMetadata.sampleRate : -1)
                  + ")");
        }
