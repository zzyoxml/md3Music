...
public abstract class MediaCodecRenderer extends BaseRenderer {
  ...
  protected void releaseCodec() {
    Log.i(TAG, "codec: release (wasPresent=" + (codec != null) + ")");
    try {
      if (codec != null) {
        codec.release();
  ...
  public void render(long positionUs, long elapsedRealtimeUs) throws ExoPlaybackException {
    ...
    if (probeNowMs - lastRenderProbeMs >= 1000L) {
      lastRenderProbeMs = probeNowMs;
      Log.i(TAG, "render: inputFormat=" + (inputFormat == null ? "null" : "set")
              + " codec=" + (codec != null) + " bypass=" + bypassEnabled
              + " outputEnded=" + outputStreamEnded + " inputEnded=" + inputStreamEnded
              + " state=" + getState() + " posUs=" + positionUs);
    }
  ...
  private void flushCodec() {
    ...
    if (diagFlushNow - diagFlushWindowStartMs > 1000L) {
      diagFlushWindowStartMs = diagFlushNow;
      diagFlushCount = 0;
    }
    if (++diagFlushCount == 20 && !diagFlushStackDumped) {
      diagFlushStackDumped = true;
      StringBuilder sb = new StringBuilder("flushCodec CHURN v3 (>20/s) stack:");
      for (StackTraceElement e : new Throwable().getStackTrace()) {
        sb.append("\n    at ").append(e);
      }
      Log.i(TAG, sb.toString());
    }
    Log.i(TAG, "codec: flush (receivedBuffers=" + codecReceivedBuffers + ")");
    try {
   ...
   */
  protected boolean shouldFlushCodecOnInsufficientCapacity() {
    return true;
  }
  ...
  private void initCodec(MediaCodecInfo codecInfo, @Nullable MediaCrypto crypto) throws Exception {
    ...
    Log.i(TAG, "codec: init " + codecInfo.name + " sr=" + inputFormat.sampleRate
        + " ch=" + inputFormat.channelCount + " pcm=" + inputFormat.pcmEncoding
        + " maxInputSize=" + inputFormat.maxInputSize);
   ...
   */
  private void feedProbe(String reason) {
    long now = getClock().elapsedRealtime();
    if (now - lastFeedProbeMs < 1000L) {
      return;
    }
    lastFeedProbeMs = now;
    Log.i(TAG, "feed: " + reason + " inputEnded=" + inputStreamEnded
        + " drainState=" + codecDrainState + " inputIndex=" + inputIndex
        + " codecReceivedBuffers=" + codecReceivedBuffers);
  }
  ...
  private boolean feedInputBuffer() throws ExoPlaybackException {
    if (codec == null || codecDrainState == DRAIN_STATE_WAIT_END_OF_STREAM || inputStreamEnded) {
      feedProbe("skip codec=" + (codec != null));
      return false;
    }
    ...
    if (inputIndex < 0) {
      ...
      if (!diagInputCapLogged) {
        ...
        Log.i(TAG, "feed: codec input buffer capacity="
            + (buffer.data != null ? buffer.data.capacity() : -1)
            + " maxInputSize=" + (codecInputFormat != null ? codecInputFormat.maxInputSize : -1));
      }
    }
    ...
    try {
      ...
      if (insuffNowMs - lastInsuffProbeMs >= 1000L) {
        ...
        Log.i(TAG, "feed: InsufficientCapacity — " + e.getMessage()
            + " bufCap=" + (buffer.data != null ? buffer.data.capacity() : -1)
            + " reconfigBytes=" + adaptiveReconfigurationBytes
            + " codecMaxInput=" + (codecInputFormat != null ? codecInputFormat.maxInputSize : -1)
      ...
      // 若连续超限帧则退化成 ~100Hz churn → 数秒静默（P0-5 已实证）。
      // v4 行为：只丢该样本，解码器继续下一帧 → 损失仅为该帧时长（~85-170ms）。
      onCodecError(e);
      readSourceOmittingSampleData(/* readFlags= */ 0);
      if (shouldFlushCodecOnInsufficientCapacity()) {
        flushCodec();
      }
      return true;
    }
    ...
    if (result == C.RESULT_NOTHING_READ) {
      ...
      feedProbe("source-empty streamToEnd=" + hasReadStreamToEnd()
          + " lastBufferInStreamPts=" + lastBufferInStreamPresentationTimeUs);
      return false;
    }
    if (result == C.RESULT_FORMAT_READ) {
    ...
    if (!hasOutputBuffer()) {
      ...
      if (outputIndex < 0) {
        ...
        if (drainNow - lastDrainProbeMs >= 1000L) {
          ...
          Log.i(TAG, "drain: no-output idx=" + outputIndex
              + " codecReceivedBuffers=" + codecReceivedBuffers
              + " inputIndex=" + inputIndex + " drainState=" + codecDrainState);
        }
        return false;
      }
