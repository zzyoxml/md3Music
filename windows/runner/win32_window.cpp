...
void Win32Window::AddTrayIcon() {
  if (tray_icon_added_ || !window_handle_) return;
  ...
  wcscpy_s(nid.szTip, L"md3music");
  tray_icon_added_ = Shell_NotifyIcon(NIM_ADD, &nid) == TRUE;
}
...
void Win32Window::ShowTrayMenu() {
  ...
  if (!menu) return;
  AppendMenu(menu, MF_STRING, 1, L"\u663e\u793a md3music");
  AppendMenu(menu, MF_SEPARATOR, 0, nullptr);
  AppendMenu(menu, MF_STRING, 2, L"\u9000\u51fa md3music");
...
Win32Window::MessageHandler(HWND hwnd,
  ...
  switch (message) {
    ...
    case WM_CLOSE:
      if (minimize_to_tray_ && !quit_requested_) {
        AddTrayIcon();
