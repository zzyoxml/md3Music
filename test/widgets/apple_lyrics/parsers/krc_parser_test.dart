...
''';
    ...
    test('8. 整行文本拼接：LyricLine.text 等于所有字文本拼接', () {
      const input =
          '[0,5000]<0,500,0>Hel<500,500,0>lo<1000,1000,0>, <2000,2000,0>World!';
      final result = KrcParser.parse(input);
...
''';
    ...
    test('只有行首时间戳但无字标签：降级保留逐行文本', () {
      const input = '[0,1000]no word tags here';
      final result = KrcParser.parse(input);
      expect(result, hasLength(1));
      expect(result.first.text, 'no word tags here');
      expect(result.first.hasWordTiming, isFalse);
    });

    test('有时间段但无文字：保留不可见占位行', () {
      final result = KrcParser.parse('[1000,2400]');
      expect(result, hasLength(1));
      expect(result.first.startTime, 1000);
      expect(result.first.duration, 2400);
