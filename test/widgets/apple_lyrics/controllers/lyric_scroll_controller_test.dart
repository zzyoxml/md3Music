...
/// 5. 普通模式动态参数（interval 100ms→140, 800ms→110）
/// 6. onUserScroll 直接修改 posY
/// 7. onUserScrollEnd 后 autoReturnMs 自动回弹（fakeAsync）
...
void main() {
  group('LyricScrollController', () {
    ...
    test('4. setSeekingMode(true) 后弹簧参数为 stiffness=90, damping=15', () {
      ...
      // 普通模式 stiffness 在 [110, 140] 之间
      expect(controller.currentStiffness,
          allOf(greaterThanOrEqualTo(110), lessThanOrEqualTo(140)));
    ...
    test('5. 普通模式动态参数：interval 100ms→stiffness=140，800ms→stiffness=110', () {
      final controller = LyricScrollController();
      controller.setViewportSize(const Size(400, 600));

      // 间隔 100ms（密集）→ stiffness=140（最灵敏）
      controller.setCurrentLine(
        0,
        isSeeking: false,
        lineHeight: 40,
      ...
      expect(controller.currentStiffness, closeTo(140, 1e-3));
      // damping = sqrt(140) * 2.2
      expect(controller.currentDamping,
          closeTo(LyricLayout.posYNormalDamping(140), 1e-9));

      // 间隔 800ms（稀疏）→ stiffness=110（最迟缓）
      controller.setCurrentLine(
        1,
        isSeeking: false,
        lineHeight: 40,
      ...
      expect(controller.currentStiffness, closeTo(110, 1e-3));
      expect(controller.currentDamping,
          closeTo(LyricLayout.posYNormalDamping(110), 1e-9));

      // 超出 clamp 范围：50ms 应等价于 100ms（→ 140）
      controller.setCurrentLine(
        2,
        isSeeking: false,
        lineHeight: 40,
      ...
      // 超出 clamp 范围：2000ms 应等价于 800ms（→ 110）
      controller.setCurrentLine(
        3,
        isSeeking: false,
        lineHeight: 40,
    ...
    test('7. onUserScrollEnd 后 autoReturnMs 自动回弹（fakeAsync 模拟时间）', () {
      fakeAsync((async) {
        ...
        // 每个 tick 推进 16ms
        const int dtMs = 16;
        // 未回弹窗口：严格小于 autoReturnMs，不应触发回弹
        final int belowReturnTicks =
            (((LyricLayout.autoReturnMs - 1) / dtMs).floor());
        for (int i = 0; i < belowReturnTicks; i++) {
          controller.tick(0.016);
          async.elapse(const Duration(milliseconds: 16));
        }
        // 尚在等待期内，未回弹
        expect(controller.isWaitingForAutoReturn, isTrue,
            reason: '未到 autoReturnMs 之前不应回弹');
        expect(controller.posY, closeTo(stableY + 80, 1.0),
            reason: '等待期内位移不应改变');

        // 继续推进超过 autoReturnMs，应触发回弹并收敛
        for (int i = 0; i < 600; i++) {
          controller.tick(0.016);
          async.elapse(const Duration(milliseconds: 16));
        }
        expect(controller.isWaitingForAutoReturn, isFalse,
            reason: '超过 autoReturnMs 后应已回弹');
        // 回弹后应收敛回 stableY
