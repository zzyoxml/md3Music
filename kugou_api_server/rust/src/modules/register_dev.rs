...
pub fn handle(q: &Value, ctx: &Ctx) -> Result<ModuleResponse, ModuleResponse> {
    ...
    // Idempotent device registration: once a dfid has been obtained and
    // persisted (device_info.json), reuse it locally instead of re-proxying
    // /risk/v2/r_register_dev on every app launch. Repeated fresh registrations
    // are what made the account appear as a pile of devices and trip risk
    // control. Registering upstream happens only on first install per app data.
    let cached_dfid = DeviceConfig::instance().get_dfid();
    if !cached_dfid.is_empty() && cached_dfid != "-" {
        return Ok(ModuleResponse {
            status: 200,
            body: BodyValue::Json(json!({ "status": 1, "data": { "dfid": cached_dfid } })),
            cookie: vec![format!("dfid={}", cached_dfid)],
